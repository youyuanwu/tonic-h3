...
- `tonic-h3-tests/src/goaway_reconnect.rs` (SF-3) — proves both directions of the
  connection-level-vs-per-stream classification with a connect-counting connector
  (`CountConnect`):
  - `quinn_goaway_triggers_reconnect` — a real quinn + h3 server (a fake `SendRequest`
    cannot be injected) sends a genuine GOAWAY via `h3::server::Connection::shutdown(0)`
    and **keeps the connection/driver alive** so the client's `driver_rx` stays pending,
    exercising the new closing-flag path rather than the pre-existing driver-ended path.
    Request #1 succeeds; a bounded probe loop then sees a request fail cleanly with the
    closing error and a subsequent request succeed via reconnection, with the connect
    count reaching **2** (a connection-level GOAWAY *does* reconnect).
  - `quinn_stream_error_does_not_reconnect` — the complementary negative case (FR-006):
    a *per-stream* error must NOT retire the shared sender. It induces a deterministic
    request-body-source failure that resets one request stream (mirroring
    `cancel_reset`'s `FailAfterOneFrame`) through an **`axum-h3` server** (an
    `axum_h3::H3Router` over quinn — the proven, CI-reliable harness, **not** a
    hand-rolled raw-`h3` server). After the server confirms it observed the stream reset
    (`H3_INTERNAL_ERROR`), the test asserts the connect count stays **1** (no reconnect),
    and that a subsequent normal request succeeds on the same connection with the count
    still **1**.

The FR-006 negative invariant (a *per-stream* error must NOT retire the shared sender —
connect count stays **1**) is enforced in code by the connection-level-only
classification (see "Connection-level vs. per-stream classification" above), and is now
covered directly by `quinn_stream_error_does_not_reconnect` (above) in addition to being
exercised indirectly by `cancel_reset::client_body_error_resets_stream`. An earlier
dedicated test for this invariant used a hand-rolled raw-`h3` server that was not reliable
on CI runners (the request hung) while passing locally, so it was removed; the replacement
above builds its connect-count assertion on top of the proven `axum-h3` test harness (as
used by `cancel_reset`) and is verified reliable under default parallel `cargo test`
without any global single-threading.
...
| `tonic-h3-tests/src/goaway_reconnect.rs` | SF-3 GOAWAY-reconnect test (`quinn_goaway_triggers_reconnect`) plus the FR-006 negative test (`quinn_stream_error_does_not_reconnect`), the latter built on an `axum-h3` server with a `CountConnect`-injecting client helper. |
