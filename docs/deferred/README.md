...
## Status of the completed / deferred items
...
| SF-1 | `RequestSender::call` can panic on URIs missing scheme/authority/path | ✅ Fixed (this PR) |
| SF-3 | A peer `GOAWAY` can pin the client to a sender that rejects new requests | ✅ Fixed (this PR) |
...
**SF-1** and **SF-3** have since been fixed (see
[`client-uri-goaway-fixes.md`](../client-uri-goaway-fixes.md)). The remaining
open should-fix items are:
...
reclassified as above. Should-fix items SF-1 and SF-3 were subsequently fixed
(this PR); the remaining should-fix and consider items were not in scope for
those cycles and are recorded here so they are not lost.
